...
index = itertools.product(range(NUM_DIV), repeat=4)


df = pd.DataFrame([], columns=['sn','population','elite_rate', 'crossover', 'mutation', 'filename', 'error'])
for c, ind in enumerate(index):
    ...
    config.GA_MAX_CROSSOVER = crossover[ind[3]]
